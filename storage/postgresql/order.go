...
func (r *orderRepo) AddOrderItem(ctx context.Context, req *models.CreateOrderItem) error {
	...
	var (
		qunatity_inStock	int
		store_id 			int
	)

	

	err := r.db.QueryRow(ctx, "SELECT store_id FROM orders WHERE order_id = $1", req.OrderId).Scan(&store_id)
	if err != nil{
		return err
	}

	err = r.db.QueryRow(ctx, "SELECT quantity FROM stocks WHERE store_id = $1 AND product_id = $2",
			store_id,
			req.ProductId,
	).Scan(&qunatity_inStock)
	if err != nil{
		return err
	}

	
	if req.Quantity > qunatity_inStock{
		newerror := fmt.Sprintf("quantity in stock = %d, you want = %d", qunatity_inStock, req.Quantity)
